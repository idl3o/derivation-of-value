...
**The mechanism is operationally closed, in Varela's exact sense.** His closure thesis has an autonomous system "operating solely on the basis of its own self-produced structures rather than on input it receives directly from the outside." A coherence mechanism scores agreement among participants' models; participants optimise toward the score; so the criterion is generated by the things it judges. That is the definition almost word for word, not a metaphor. The whitepaper's sentence was apter than its author knew.

**Result 1 — the fiction space has dimension d** (`code/fiction_space.py`). Hansen & Ghrist showed the sheaf Laplacian implements consensus, so a coherence-maximising network is a dynamical system with a known attractor set: ker Δ ≅ H⁰. Measured at d = 3, five random starts converge to five *distinct* global sections (residuals 10⁻¹⁵) whose kernel projections span three dimensions — they **exhaust** the fiction space. Five worlds, each perfectly coherent, and the mechanism has no preference among them. The initial condition chose. **Corollary 4.2, uncomfortable:** the fiction space is d, the stalk dimension, so *richer models buy more room to be consistently wrong* — richness is a ceiling on security and a floor under drift, and they are the same parameter.

**Result 2 — closure costs d scalars, once.** Pinning scalars at a *single vertex* reduces the fiction space one dimension at a time; d pins collapse it entirely, because a global section of a connected coherent sheaf satisfies x_v = R_vᵀc and is determined by its value at any one point. Prop 5.2: the requirement is d **per connected component** and independent of network size — a network ten times larger needs no more contact with the world; one that has split in two needs twice as much. Connectivity is therefore a coupling requirement as well as (per the H⁰ duplication result) a Sybil requirement.

**The turn (§6).** *Gauge-Fixing*'s four anchors fix backdating, grinding, duplication and identity-minting — every one of them **provenance**. The fiction space is a freedom of **content**, and no gauge anchor touches it. The prohibition is explicit: the physical anchor must be consumed "only as randomness and timing, never once cited as a certificate of order," precisely so it cannot swell into a proxy. So the same discipline that keeps the beacon honest is what prevents it supplying content. The anchors make the fiction unforgeable and well-attributed; they do not make it true. Volume V's residue does not rescue it either, having been shown there to be negentropy *with provenance fixed*. **The program has chosen closure knowingly and never priced the choice** — closure against d scalars of contact.

**The proposed escape, marked as conjecture.** Kar-Coin's inward axis attests by artifact, and *measuring a lattice is not asking anyone*. Such a reading is content-bearing and involves no party required to be honest. If one stalk could be taken from a physical measurement of a shared world, Prop 5.2 says that closes the fiction space for the whole component. §8.2 states what a construction would need. It suggests the inward axis was never only about capacity — it may be the program's only available form of structural coupling.

Declined: that closure is a pathology (it is how living systems hold identity; the claim is only that it does not track truth); Luhmann, enactivism and second-order cybernetics, none imported; that d scalars are free — cheap in quantity, not necessarily in kind, since a pinned value from a party asked to be honest is an oracle by Def 2.3 and the whole escape turns on measurement not being testimony; that the fiction space is the whole of closure (it is measured at the attractor, and deployed systems live away from it); and anything built.
