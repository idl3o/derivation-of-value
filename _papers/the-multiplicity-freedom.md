...
eyebrow: "An Anthology · Paper · v0.5"
...
blurb: "The program has named the Sybil attack four times without using the word. Naming it lets the anchors be read as a proof: conjunction-gating amplifies a resource floor's Sybil resistance by a factor of 1 + (K−1)ι, and by nothing at all if the projections are redundant. The gate caps the adversary's fleet; only convexity above it makes splitting a loss — and diminishing returns, the conventional anti-whale choice, subsidises Sybils. v0.4 adds a second way for richness to buy nothing: a projection contributes in proportion to its own trace gap, so a perfectly independent one that is free to forge amplifies no more than a redundant one — and the specimen has been measured. v0.5 records §8.4 answered by *The Budget Across Levels*: the criterion for a Sybil refinement is correct and not checkable from the reading, and the level of presentation changes receipts alone."
...
**Corollary 5.4 (independence is *not* the whole of it).** New in v0.4, and it narrows Corollary 5.2's headline. *Sign and Work* Proposition 4.1 replaces each projection's effective cost γᵢ with τᵢγᵢ, where τᵢ is that projection's trace gap. A projection therefore contributes to Γ **in proportion to its own trace gap**, whatever its independence, and

> at τᵢ ≈ 0 a projection adds approximately nothing to Γ even at ιᵢ = 1.

So there are two ways for richness to buy nothing, not one. Corollary 5.2 names redundancy; this names forgeability. A perfectly independent projection whose trace an adversary can lay for free amplifies the resource floor by no more than a redundant one does.

**And the specimen exists.** `code/temporal_iota.py` measures a temporal persistence projection with ι(ker \| persist) = 1.000 — maximally independent, at every coalition size — and a trace gap of approximately zero, since a coalition that simply never changes its restriction maps outscores honest participants who update theirs. It satisfies Corollary 5.2's condition perfectly and contributes essentially nothing to Theorem 5.1's cap. The projection the framework's §6 anticipated as the second half of its worked instance is exactly this specimen.

**A caveat on ι itself, which affects every result in this section.** Theorem 4.2 and everything downstream treat ι as a scalar constant of the substrate and projection pair. Measured, it is not stable: ι(persist \| ker) returns 0.28, 0.86 and 1.03 as attack intensity rises, a spread of three quarters. If ι depends on the attack that measures it, then Γ = γ(1 + (K−1)ι) is not well-defined until an adversary is named. Three data points with a monotone trend is suggestive rather than established, and the conservative repair is available and should be adopted in any deployment: **read ι as an infimum over the attacker class rather than a value**, since the adversary chooses the attack and will choose the one that makes ι smallest. Under that reading the bounds of this section stand and are simply weaker than the point estimates suggest.
...
**8.4. ~~Sybils across levels.~~ Answered, unkindly — *The Budget Across Levels* v0.2 §§5–6.** The criterion is correct — an interface with both sides controlled is interior to the coalition and evidences nothing — and it is not checkable from the reading: the fiction glues with residual zero, a two-sided gate refuses that, and honest-level noise added at no cost defeats the two-sided gate. Refining or coarsening a coalition's presentation changes its receipts and not its native cost, and the reading cannot tell either from composition. *Original statement follows.* A holon presenting as many sub-holons is either legitimate composition or a Sybil attack, and the distinction cannot be made from within either level. The natural criterion — that a refinement is Sybil when the adversary controls both sides of an interface — is precisely the structure that the program's adversarial experiment found effective where random collusion was not. Whether that criterion is checkable is open, and it is where this paper and *Gluing the Gates* meet.
