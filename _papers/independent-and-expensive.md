...
blurb: "The framework's reordered central question — is any projection both independent and expensive to forge? — answered in a model small enough to prove things in: independence and trace gap are claims on one budget, the substrate's paid work, and disjoint claims on a finite pool cannot both be large. Richness partitions the budget; nothing multiplies it. The escape is an anchor per projection, and it is an escape by purchase, not by cleverness — measured on the sheaf, where the dial that buys persistence its gap spends its independence in the same stroke. v0.2 records that §8.2 and §8.4 have been run, in that order, by *The Second Pool*: the purchase returns the arithmetic to the decimal, and the purchased projection evidences its anchor. v0.3 records that §8.3 has been run by *The Budget Across Levels*: receipts sum, interfaces nest, and the evidencing budget of a holarchy is its finest level's."
...
**8.2. ~~The two-pool sheaf.~~ Built — *The Second Pool* v0.1 §§5–6.** The prediction held to every decimal: ι = 1.000 both ways at every positive setting of the two dials, both gaps positive, summing to one minus the reconciliation share. The purchased projection evidences its anchor and nothing else (survival zero by construction), and the chain transports the static anchor's value for nothing, so the two anchors are independent as receipts and not as values. *Original statement follows.* Give persistence an anchor of its own, disjoint from the kernel's — a per-epoch encoding on the temporal structure that the static encoding does not contain. The model predicts ι(ker\|persist) = 1 restored with both gaps positive and summing to at most one. Building it would exhibit, on the program's own substrate, the first projection pair that is independent *and* expensive — at a price the budget makes explicit.

**8.3. ~~Does the budget compose?~~ Answered — *The Budget Across Levels* v0.2 §§3–5.** Sum on the anchor side, union on the native side: receipts are disjoint per level and add, and every one is idle; interfaces nest, so the evidencing budget of a holarchy is its finest level's however many levels it has, and *Gluing the Gates* Proposition 6.1's minimum is zero on the native side for every block coalition. Nesting neither divides the evidencing budget nor partitions it. *Original statement follows.* *Gluing the Gates* showed richness does not compose across a holarchy and *Sign and Work* §8.4 asked the same of τ. The budget's composition is the sharper form of both: whether a holarchy's pool is the sum of its levels' pools, or the minimum, decides whether nesting divides the budget or merely partitions it, and Proposition 6.1 of that paper suggests the unkind answer.

**8.4. ~~The signal that survives its anchor.~~ Answered in the negative for the class — *The Second Pool* v0.1 §§3–4.** Under induced maps the overlap reading measures the uniformity of the frames' motion, uniform motion is a time-dependent gauge, and coherent change and no change score identically; the honest excess over the anchor-only class is indistinguishable from zero across ten paired seeds. *Original statement follows.* §5's unanticipated finding: the epoch-keyed anchor dominates the eigenspace the persistence projection reads. Whether any temporal reading carries a signal that survives its own anchor is *Sign and Work* §8.1 asked on the time axis, and it is prior to 8.2 — a second pool is worth nothing to a projection whose signal its anchor has already eaten.
