...
eyebrow: "Whitepaper · v0.5 · working draft"
permalink: /whitepaper/
label: "Whitepaper"
blurb: "A Goodhart-asymptotic incentive mechanism for decentralized inference. Sheaf-theoretic, with a spectral extension via Proof by Resonance. v0.5 absorbs three measurements against the spectral layer: its static form is redundant with the kernel, its temporal form is independent but free to forge, and the autopoietic-cult defence is inverted — a cluster that has stopped scores five times an honest network that has not."
...
- *Integrated.* A duplicate attached to the original's own neighbours adds nothing to H⁰, at any tolerance. The rank intuition holds.
- *Scattered.* Duplicates forming components of their own each add d. Measured at d = 3, the adversary's marginal credit runs +3, +6, +12, +24 for one, two, four and eight copies — exactly dN, unbounded, and identical at machine precision and at every tolerance tested. **Duplication-boundedness fails outright.**
- *Bridged.* Duplicates chained to the complex through a single edge stay in one component, so H⁰ is exactly d and the property holds *exactly*. But at ε = 0.05, eight bridged copies register three additional approximate sections. The attachment is genuine and weak; its algebraic connectivity is small; and a kernel tolerance that does not respect the complex's spectral gap counts a weakly-attached appendage as a section.
...
- **~~Spectral Goodhart asymmetry.~~ Refuted in the static case, hollow in the temporal case. v0.5.** The v0.2 claim was that faking r_spec additionally requires producing outputs whose marginal contribution to the full spectrum has a target shape across multiple epochs, and that "faking the eigenvalue distribution of Δ¹ across time requires reproducing the dynamical structure of an actually-coherent system." Both halves have now been measured (`code/independence.py`, `code/temporal_iota.py`; *Combination Proofs* v0.6 §6). *Statically*, the spectral projection is not independent of the kernel at all: against a structure-aware coalition ι ≈ 0 at every depth tested, because in a snapshot the spectral dimension is measuring the coherent fraction of the substrate and so is the kernel count. Faking H⁰ delivers the spectrum free. *Temporally*, the projection is genuinely independent — ι(kernel \| persistence) = 1.000 at every coalition size — and the independence buys nothing, because the coalition need not reproduce any dynamics: it holds its restriction maps fixed. **There is no shortcut to be found because none is needed.** The asymmetry claimed here does not hold in the form stated, and claim (1) of §1.2 should not be read as resting on it.

- **~~Autopoietic-cult defence.~~ Inverted. This is the most serious correction in v0.5.** The v0.2 argument was that a coordinated cluster passing instantaneous predictive validity would still fail on spectral signature, "because matching the latter requires the cluster to reproduce not only the right answers but the right *dynamics of arriving at* the right answers." The measurement runs the other way. Honest participants **update their models**, so an honest network's low-eigenvalue subspace *moves* between epochs; a closed cluster that has stopped taking input has a subspace that does not move at all. Scored on persistence, the frozen cluster reaches **five times** the honest network's progress at no coherence work whatever. An autopoietic cult is, by *A Consistent Fiction*'s own definition, a system operating solely on its self-produced structures — which is to say a system that has **stopped**. A temporal projection therefore does not defend against the cult; it **pays a premium for being one**, and the second line of defence claimed here is an incentive pointed at the pathology it was introduced to detect. Any deployment scoring temporal structure must score *coherent* change rather than mere persistence, and no formulation achieving that is offered in this document.

- **The reward's own trace gap, and it has a ceiling below soundness. v0.5.** *Sign and Work* v0.3 §5 measures τ = forging cost / earning cost for exactly the reading r_coh uses, and both of its results are ceilings. Under anchors that constrain the mechanism's *output*, τ ≤ 1 − b₁/|E|, with equality only where the cycle rank vanishes and the score is vacuous — and the cheapest accepted forgery is *free*, since setting every frame equal yields a perfectly coherent, entirely contentless configuration. Under anchors that constrain *production*, τ approaches 1 without attaining it, the shortfall being precisely the reconciliation term. The composite finding is that **the coherence layer contributes nothing to forgery resistance; whatever gap this mechanism has is bought by its anchors.** That is not a refutation of the construction — §1.2's multiplicative claim was always a claim about the conjunction, and the anchors are part of the conjunction — but it relocates where the security lives, and this document has consistently written as though the sheaf were carrying it.
