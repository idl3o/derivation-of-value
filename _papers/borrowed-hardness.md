...
That last sentence was written as a thing noticed, which is a weaker act than it sounds, since a reader is entitled to ask whether the noticing was of the world or of the noticer. It has since been measured, in one place and by one instrument, and it survived. Take a mechanism that delivers coherence across a scope — two participants at one end, a whole network at the other, two orders of magnitude between them — and ask what contact with the world costs it. The cost is of the same sign and the same order at both ends. The move does recur.

What the measurement adds, and what the sentence above could not have known, is a *direction*. The cost is not merely present at every scale; it is **larger at the small end**. Two participants pay more, in the coin of their own satisfaction, for contact with the world than a whole network does. The recurrence is therefore not a symmetry but a flow — the quantity runs as the scale is changed, in the manner of a coupling that is not fixed but scale-dependent, which is the shape the physicists gave to exactly this kind of self-similarity and which this anthology has so far only gestured at. One measurement on one substrate is not a theory of that flow. It is, however, the first evidence that there is one to have, and the first time this coda's claim has been anything other than a claim.
