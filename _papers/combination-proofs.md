...
eyebrow: "Framework · v0.6"
permalink: /combination-proofs/
label: "Framework"
blurb: "A framework for Goodhart-asymptotic mechanism design. Reward gated on the conjunction of verifiably independent projections of a structural substrate. v0.4 reports that independence was never the binding constraint: a projection can be perfectly independent of the kernel and still free to forge, and §7.1 has been stating one of two requirements. v0.5 records the amendment *Independent and Expensive* made to Proposition 4.2: publicity-positivity is non-decreasing always and strict only while unclaimed paid work remains. v0.6 amends Definition 2.6 per *One's Own Anchor*: each projection must be approximately independent of its own anchor as well as of the others, or the mechanism is its anchors."
status: "v0.6 · working draft"
order: 2
---

*v0.6 · working draft. v0.1 the initial statement of the framework. Definitions established; multiplication and publicity-positive claims stated with conditions; substrate richness defined; Proof of Coherence with its spectral extension introduced as the worked instance; open problems and program outlined. v0.2 splits the civilisational-capacity substrate (§8) into outward and inward projection families and records the resulting attestation asymmetry — extensive projections tend to require an oracle, intensive ones tend to supply their own verifier — placing the verifiability boundary of §7.2 inside a single substrate for the first time. §7.3 records the program's first measurement of ρ — a fractional spectral dimension, continuous in structure — and the refutation of the accompanying gap-hierarchy conjecture. v0.3 extends §4 from publicity-positive to adversary-positive security: residue, harvestable projections, and the conditions under which failed forgery subsidises the substrate it attacks. v0.4 corrects §6's independence claim in both halves — the static spectral projection is not independent, and the temporal one is independent and free to forge — and rewrites §7.1 around the resulting change of view: independence is one of two requirements on a projection, the framework has stated only that one as an open problem, and the two appear to pull against each other. ι is measured in the direction that was missing, found asymmetric, and found to vary with the attack that measures it. v0.5 amends Proposition 4.2(ii) in place, per *Independent and Expensive* v0.2 Corollary 3.6: the marginal fake-cost of a published projection is at most the unclaimed remainder of the substrate's paid work, so strict publicity-positivity saturates at the pool. v0.6 amends Definition 2.6 in place, per *One's Own Anchor* v0.3 Corollary 3.3: a projection's native share is its independence from its own anchor times its trace gap, so the conjunction's projections must be approximately independent of their own anchors as well as pairwise, or Γ is the anchor work exactly.*
...
**Definition 2.6 (Combination Proof).** A mechanism M is a *Combination Proof of order K* if there exist projections π₁, …, π_K of its substrate, each intrinsically verifiable, ~~each pairwise approximately independent~~ each pairwise approximately independent **and each approximately independent of its own anchor** — Definition 2.5 applied with the anchor's receipt check as the conditioning projection; amended in v0.6 per *One's Own Anchor* v0.3 Corollary 3.3, without which a mechanism of any order may have fake-cost equal to its anchor work exactly — such that M's reward function is
...
(ii) ~~the attacker's fake-cost satisfies C(M′ ; A) > C(M ; A), with the gap given by the marginal fake-cost of π_{K+1}.~~ **Amended in v0.5.** The attacker's fake-cost satisfies C(M′ ; A) ≥ C(M ; A), with the gap given by the marginal fake-cost of π_{K+1}, and that marginal is at most the *unclaimed remainder* of the substrate's paid work — zero once the claims of π₁, …, π_K exhaust it (*Independent and Expensive* v0.2 Corollary 3.6, from its Theorem 3.4). Publicity-positivity is therefore non-decreasing always and strict only while unclaimed paid work remains. The research literature is a security accumulator with a ceiling, and the ceiling is the substrate's work content.
...
**That paragraph is now half refuted and half hollow, and v0.4 corrects it rather than deleting it.** It makes two claims, and both have been measured (`code/independence.py`, `code/temporal_iota.py`).

*The static half is false.* Against a structure-aware coalition the non-zero spectrum and the kernel move together: at the threshold where a conjunction gate would plausibly sit, ι ≈ 0 at every depth tested, because in a static snapshot the spectral dimension is measuring the coherent *fraction* of the substrate and so is the kernel count. Two instruments, one quantity. Faking the kernel delivers the spectral dimension free, and the static spectral projection is **not** independent in the sense of Definition 2.5.

*The temporal half is true and useless.* Building the epochs the paragraph invokes, and scoring persistence as the overlap of consecutive low-eigenvalue subspaces, gives ι(ker \| persist) = 1.000 at every coalition size — the projection is independent, exactly as claimed. But "no shortcut to compute" is wrong, because the coalition need not compute anything: it holds its own restriction maps fixed and never agrees about anything, and since honest participants *update* their models, the coalition that changes nothing outscores the honest one by a factor of five. The shortcut to sustained structure is not to sustain anything. It is to stop.

The correction that matters is not to either half but to the sentence's implicit assumption that independence was the difficulty. It was not; see §7.1.
...
*Four measurements, and a change of view about what this problem is.* Three attack designs have been aimed at ι and the third succeeded (`code/independence.py`, `code/iota_asymmetry.py`, `code/temporal_iota.py`).

**The measured values.** ι(dim \| ker) ≈ 0.08 against a structure-aware coalition on the static complex. ι(ker \| persist) = **1.000**, at every coalition size, against a temporal persistence projection — the first solid measurement in this direction, obtained only by building the epochs rather than attacking the snapshot harder. Two earlier designs failed and the failures are kept in the code: sparsification shatters the complex and contaminates the kernel proxy with component count, and degree-preserving rewiring stays connected but moves the spectral dimension only eight per cent.

**ι is asymmetric on this pair, at 0.277,** which clears the threshold set for the question and indicates that the right object is a **divergence rather than a metric**. The asymmetry encodes attack ordering and should be carried rather than quotiented away, which is a constraint on any Fisher-information formalisation of the kind proposed above.

**ι may not be a scalar at all.** ι(persist \| ker) returns 0.28, 0.86 and 1.03 as attack intensity rises — a spread of three quarters across the range. This paragraph types ι as a function of substrate and projection pair; on this evidence it is also a function of the attack that measures it, which would mean the multiplication claim's leading-order statement and *The Multiplicity Freedom*'s Γ = γ(1 + (K−1)ι) are both using a quantity that is not well-defined independently of an adversary. Three data points with a monotone trend is suggestive and not established, and settling it is now the sharpest form of this open problem.

**And independence was never the binding constraint.** The temporal projection is independent of the kernel *and free to forge*: holding still costs nothing and outscores honest updating, so its trace gap τ (*Sign and Work* Def 3.3) is approximately zero, and by Proposition 4.1 of that paper a projection with τ ≈ 0 inflates the adversary's fleet without bound. A projection must clear **both** requirements, and this framework has stated only one of them as an open problem. The two also appear to pull against each other: a projection independent of coherence is one that coherence does not constrain, and a projection coherence does not constrain is one an adversary satisfies without doing coherence work. The static spectral projection failed on ι and inherited coherence's τ; the temporal projection passes ι and has no τ. On two data points that is a conjecture rather than a finding, but if it holds then admitting a third projection is harder than §7.1 has ever suggested, and the difficulty is not the one §7.1 names.
