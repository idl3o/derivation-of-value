...
**7.1. Measure τ for more projections.** One is now measured, and it came out at the worst possible value. The H⁰ projection of *Proof of Coherence*, scored without a participation requirement, has **τ ≈ 0**: an adversary registering a disconnected vertex adds d to the score at essentially no cost, since an isolated stalk is unconstrained and no work is performed at all (`code/h1_cohomological.py`; §4.2.2 of that paper). By Proposition 4.1 the Sybil cap is inflated by 1/τ, which is to say without bound — and this is the same statement as that paper's participation condition C1, reached from the trace side. A projection is only as sound as the cheapest way to make it register. Estimating τ for the *spectral* projection, and for a participation-constrained H⁰, is the natural next measurement.
