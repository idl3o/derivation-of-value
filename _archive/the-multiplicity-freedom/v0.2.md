...
**H1 is discharged for coherence substrates, and not by any credit rule.** Measurement (`code/h1_duplication.py`) finds it satisfied under every scheme *Proof of Coherence* considers — marginal removal, Shapley, and both forms of provenance-weighting — for every duplication count tested. The reason is structural: the coherence functional there is a rank, hence a matroid rank function, and a duplicate adds no rank. **Duplication-for-profit is resisted by the substrate, not purchased from the allocation scheme.** This generalises as far as the submodularity does: any coherence functional for which duplicates are redundant supplies H1 for free, and a substrate whose functional rewards redundancy would not, whatever credit rule were laid over it.
...
**That H1 holds generally.** It is discharged for coherence substrates, where the functional's rank structure supplies it under every credit rule tested, and the argument extends only as far as that submodularity does. For substrates whose functional rewards redundancy — and none has been ruled out — H1 must be established separately, and until it is the theorems there describe a mechanism nobody has built.
...
**8.2. H1 beyond rank.** Settled for coherence substrates in *Proof of Coherence* §4.2.1, where it turns out to follow from the functional being a rank rather than from any choice of credit rule. The open form is now narrower and sharper: for which substrate functionals does duplication-boundedness fail? A functional that rewards redundancy would break every bound in this paper, and the framework currently places no condition on functionals that would exclude one.
