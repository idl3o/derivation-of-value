...
**7.1. Exhibit a Combination Proof as an open game.** The concrete work Claim 3.1 needs: strategy sets, play function, coutility, and a demonstration that conjunction-gating survives composition when read off the backward leg. This is the paper's first debt and it is tractable. **It is also more tractable than v0.1 knew.** The obstacle is that a mechanism paying a reward has an awkward relationship to coutility — utility returned to the environment is precisely what a payout mechanism does *not* obviously produce, since its output goes to its own participants. A mechanism that instead *delivers* a service returns something to its environment by construction, so coutility is its output type rather than a quantity to be identified. Whether that makes the exhibition easy or merely possible is untested, but the type-matching is no longer the hard part, and any attempt at this debt should start there. See `_plan/service-reframe.md`.
