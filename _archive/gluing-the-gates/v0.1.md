...
**8.6. Is interface capture a cohomological condition? — new in v0.2.** This paper's organising claim is that composition failures live at interfaces, because interfaces are the part of the structure nobody designed and nobody audits. A literature the program had not consulted has been attacking a special case of that for twenty years, and a prior-art check run 2026-08-06 turned it up: Singh, Castro, Druschel and Rowstron's *Eclipse Attacks on Overlay Networks* (INFOCOM 2006) studies exactly the configuration in which a coalition arranges for a correct node to peer only with coalition members, thereby mediating all of its communication. Two of their results matter here.

First, **defences against Sybil attacks do not prevent eclipse attacks**, because an adversary may manipulate the overlay's maintenance algorithm rather than merely mint identities. Restated in this program's terms: bounding an adversary's *budget* does not bound its *position*, and every bound the corpus owns is of the first kind. The Kademlia literature supplies the sharp version — an eclipse can succeed with as few as eight strategically placed nodes.

Second, and the reason this belongs in an open-problems section rather than a related-work one: **their defence is that nodes anonymously audit one another's connectivity.** That is a structural check on the graph, arrived at independently, in a setting where the graph is a routing overlay rather than a holarchy. This paper's machinery is a sheaf over exactly such a graph. The question is whether an eclipse — a node all of whose interfaces have been captured — is detectable as a *cohomological obstruction* rather than by a degree audit. If it is, the invariant of §4 earns a use in a field that has been solving the problem by other means, and this paper's interface claim acquires its first worked instance outside the corpus. If it is not, that is worth knowing too, since it would bound what the obstruction can see.
